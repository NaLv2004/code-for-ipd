...
void system2D_LSD_epdet(double* BER_array, double* FER_array)
{
	...
	for (double nSNR = Min_SNR; nSNR <= Max_SNR; nSNR += SNR_step)
	{
		...
		while (ENUMERATE_CODEWORD) {
			...
			// 最小重量码字存储
			const int MAX_MIN_CODWORDS = 1E5; // 假设最多1000个最小重量码字
			...
			int min_weight = N + 1; // 初始化为最大可能重量+1
			int min_count = 0;
			...
			for (long long info = 1; info < total_combinations; info++) {
				...
				if (current_weight < min_weight) {
					...
					if (min_count < MAX_MIN_CODWORDS) {
						...
						std::cerr << "警告：超过最小码字存储容量" << std::endl;
					}
				}
			}

			// 输出结果
			std::cout << "最小重量: " << min_weight << std::endl;
			std::cout << "最小重量码字个数: " << min_count << std::endl;
...
void system2D_LSD_epdet_with_CRC(double* BER_array, double* FER_array)
{
	...
	int*** tried_codewords_h = new int** [Nv];  // 第一维：Nv个指针

	for (int i = 0; i < Nv; ++i) {
		tried_codewords_h[i] = new int* [n_samples];  // 第二维：每个第一维元素指向n_samples个指针

		for (int j = 0; j < n_samples; ++j) {
			// 第三维：每个第二维元素指向Nh个整数
			tried_codewords_h[i][j] = new int[Nh]();  // 末尾的()进行值初始化（0）
		}
	}

	int*** tried_codewords_v = new int** [Nh];  // 第一维：Nv个指针

	for (int i = 0; i < Nh; ++i) {
		tried_codewords_v[i] = new int* [n_samples];  // 第二维：每个第一维元素指向n_samples个指针

		for (int j = 0; j < n_samples; ++j) {
			// 第三维：每个第二维元素指向Nh个整数
			tried_codewords_v[i][j] = new int[Nv]();  // 末尾的()进行值初始化（0）
		}
	}
...
#pragma omp parallel for
	...
	for (int i = 0; i < Nv; ++i) {
		for (int j = 0; j < n_samples; ++j) {
			delete[] tried_codewords_h[i][j];  // 释放第三维的整数数组
		}
		delete[] tried_codewords_h[i];  // 释放第二维的指针数组
	}
	delete[] tried_codewords_h;  // 释放第一维的指针数组
	for (int i = 0; i < Nh; ++i) {
		for (int j = 0; j < n_samples; ++j) {
			delete[] tried_codewords_v[i][j];  // 释放第三维的整数数组
		}
		delete[] tried_codewords_v[i];  // 释放第二维的指针数组
	}
	delete[] tried_codewords_v;  // 释放第一维的指针数组
	delete[] ep_extr_var_omp;
	delete[] y_mod_real;
	delete[] x_mod_tmp;
	delete[] y_mod_tmp;
