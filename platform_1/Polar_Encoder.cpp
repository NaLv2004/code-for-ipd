...
void sub_sort(float* FilterArray1, int* Posit, int len);//�Ӵ�С���в������Ӧ���±�
double phi(double t)
{
	if (t < 0.867861)
...
// RM-GA construction:
//   - Primary criterion: row weight of polar generator (RM rule, same as RM_codeconstruction).
//   - Tie-break inside the boundary tier (the tier where row-weight ties cross the
//     K-th info-bit boundary): pick channels with the smallest GA z-value
//     (i.e. most reliable bit-channels in the GA sense).
// Output: full ordering placed in best_channel; the first K entries are the chosen
// information bits, the remaining N-K entries are the frozen positions.
void RM_GA_codeconstruction(int N, int K, int** GG, float sigma, vector<int>& best_channel)
{
	// 1) Row weights w[i] (RM weight).
	std::vector<int> w(N, 0);
	for (int i = 0; i < N; i++) {
		int s = 0;
		for (int j = 0; j < N; j++) if (GG[i][j]) s++;
		w[i] = s;
	}

	// 2) GA z-values z[i] (smaller = more reliable). Same recursion as GA_codeconstruction.
	int m = (int)(log((double)N) / log(2.0));
	std::vector<float> z(N, 0.f);
	const float alpha = -0.4527f;
	const float gamma = 0.8600f;
	for (int i = 0; i < N; i++) z[i] = 2.0f / ((float)sigma * (float)sigma);
	for (int l = 1; l <= m; l++) {
		int o1 = (int)pow(2.0, m - l + 1);
		int o2 = (int)pow(2.0, m - l);
		for (int t = 0; t < (int)pow(2.0, l - 1); t++) {
			float T = z[t * o1];
			double v = phi_inv(1.0 - std::pow(1.0 - phi(T), 2.0));
			z[t * o1] = (float)v;
			if (z[t * o1] == HUGE_VAL)
				z[t * o1] = T + (float)M_LN2 / (alpha * gamma);
			z[t * o1 + o2] = 2.0f * T;
		}
	}

	// 3) Sort all indices: primary by row weight DESC, secondary by z ASC.
	// With this lexicographic order, the first K entries are exactly the RM-GA
	// information set: any boundary-tier ties are broken by GA reliability.
	...
	for (int i = 0; i < N; i++) best_channel[i] = order[i];
}
